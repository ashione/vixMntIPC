...
void child_receiver() {
   ...
   ILog("timeval size %d ; msg_datasize %d", sizeof(struct timeval),
        receiveMsg->msg_datasize);
...
#if defined(__cplusplus) && __cplusplus >= 201103L
      VixMntMsgData *timeMsg = new VixMntMsgData(
         VixMntMsgOp::MntWrite, sizeof(struct timeval) + strlen(msg_name), buf);
#else
      VixMntMsgData *timeMsg = new VixMntMsgData(
         MntWrite, sizeof(struct timeval) + strlen(msg_name), buf);
#endif
